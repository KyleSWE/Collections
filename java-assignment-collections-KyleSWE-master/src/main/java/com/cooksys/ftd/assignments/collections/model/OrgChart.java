...
public class OrgChart {
    ...
    public boolean addEmployee(Employee employee) {
        if(employee == null || orgChart.contains(employee)) {
        	...
        	if(employee.hasManager()){
        			...
        			for(Manager m : employee.getChainOfCommand()) {
        				orgChart.add(m);
        			}//end for
    ...
    public Set<Manager> getAllManagers() {
        ...
        for(Employee em : getAllEmployees()) {
        	if(em instanceof Manager) {
        		managerSet.add((Manager)em);
        	}
        }//end for
        	return managerSet;
    }//end getAllManagers()
    ...
    public Set<Employee> getDirectSubordinates(Manager manager) {
    	...
    	if(!(orgChart.contains(manager))){
    		...
    		for(Employee em : getAllEmployees()) {
    			if(manager.equals(em.getManager())) {
    				subSet.add(em);
    			}//end if
    		}//end for
    	}//end else
    ...
    public Map<Manager, Set<Employee>> getFullHierarchy() {
        Map<Manager, Set<Employee>> newEmpSet = new HashMap<>();
        
        for(Manager m : getAllManagers()) {
        	newEmpSet.put(m, getDirectSubordinates(m));
        }//end for 
        return newEmpSet;
    }//end getFullHierarchy()
